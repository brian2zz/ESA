<?php

namespace App\Http\Controllers;

use App\Http\Controllers\Controller;
use App\Models\kandungan_obat;
use App\Models\stok_opnam;
use App\Models\laporan_opnam;
use App\Models\kandungan;
use App\Models\dashboard;
use App\Models\dosis;
use App\Models\obat;
use App\Models\pbf;
use App\Models\dosis_obat_detail;
use App\Models\satuan_obat_detail;
use App\Models\obat_kandungan_detail;
use App\Models\golongan_obat;
use App\Models\dokter;
use App\Models\gudang_obat;
use App\Models\kategori_obat;
use App\Models\pengeluaran;
use App\Models\pengadaan;
use App\Models\penerimaan;
use App\Models\pabrik_obat;
use App\Models\satuan_obat;
use App\Exports\laporan_obatED;
use App\Exports\laporan_stok_opnam;
use App\Exports\laporan_penerimaan;
use App\Exports\laporan_pengeluaran;
use App\Exports\laporan_pengadaan;
use App\Models\detail_penerimaan_obat;
use App\Models\detail_pengeluaran_obat;
use App\Models\kategori_pengeluaran;
// use App\Exports\dashboard;
use Illuminate\support\Facades\DB;
use Maatwebsite\Excel\Facades\Excel;
use Carbon\Carbon;
// use Auth;
use Illuminate\Support\Facades\Auth;

class WellyadminController extends Controller
{
	
	    // Dashboard
    public function dashboard_1()
    {
      
     
        $page_title = 'Dashboard';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
        $active="active";
        $event_class="schedule-event";
        $button_class="btn-primary";
        $action = __FUNCTION__;
		
        return view('dashboard.index', compact('page_title', 'page_description','action','logo','logoText','active','event_class','button_class'));
    }
    
	   // Page Analytics
    public function patient()
    {
        
        $page_title = 'Patient';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
		
        $action = __FUNCTION__;

        return view('dashboard.patient', compact('page_title', 'page_description','action','logo','logoText'));
    }
	
	     // Patient Details
    public function patient_details()
    {
        $page_title = 'Patient Details';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
		
        $action = __FUNCTION__;

        return view('dashboard.patient_details', compact('page_title', 'page_description','action','logo','logoText'));
    }

	    // Doctor
    public function doctor()
    {
        $page_title = 'Doctor';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
		
        $action = __FUNCTION__;

        return view('dashboard.doctor', compact('page_title', 'page_description','action','logo','logoText'));
    }
	    // Doctor Details
    public function doctor_details()
    {
        $page_title = 'Doctor Details';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
		
        $action = __FUNCTION__;

        return view('dashboard.doctor_details', compact('page_title', 'page_description','action','logo','logoText'));
    }
	    // Review
    public function reviews()
    {
        $page_title = 'Review';
        $page_description = 'Some description for the page';
        $logo = "images/logo.png";
        $logoText = "images/logo-text.png";
		
        $action = __FUNCTION__;

        return view('dashboard.reviews', compact('page_title', 'page_description','action','logo','logoText'));
    }


        // Dashboard
    
    public function dashboard_2()
    {
        $time = Carbon::now()
                ->format('Y-n');
        $current_time  = Carbon::now()
                ->format('F, Y');
                $data= [];
        for($i = 0; $i <6; $i++){
            $time_expired = Carbon::now()
                    ->addMonth($i)
                    ->format('Y-n'); 
            $data[$i]=db::table('detail_penerimaan_obat')->where('tanggal_expired_obat','like',$time_expired.'%')
                    ->orWhere('tanggal_expired_obat','like',$time.'%')
        
                    ->update([
                        'expired_status'=>'true'
                    ]);
        }
        // dd($data);      
        
        $dashboard = detail_penerimaan_obat::leftJoin('stok_gudang','detail_penerimaan_obat.id_detail_penerimaan_obat','stok_gudang.id_detail_penerimaan')
                                ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                                ->leftJoin('penerimaan','detail_penerimaan_obat.id_penerimaan','penerimaan.id_penerimaan')
                                ->where('tanggal_penerimaan','like','%'.$current_time)
                                ->get();
                                
        $pengeluaran = detail_pengeluaran_obat::select(DB::raw('*,sum(jumlah_obat_keluar) as total_obat_keluar'))->leftJoin('stok_gudang','detail_pengeluaran_obat.id_obat_pengeluaran','stok_gudang.id_detail_pengeluaran')
                            ->groupBy('batch_number')->get();
        $page_title = 'Dashboard';
        $page_description = 'Some description for the page';

        foreach($dashboard as $data){
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }
        
        $action = __FUNCTION__;
        // dd($pengeluaran);
        return view('dashboard2.dashboard2',[
            
            'dashboard'=>$dashboard,
            'data_pengeluaran'=>$pengeluaran,
            'tanggal'=>$current_time
            // 'data_export'=> $data_export,'data_dashboard'=>$data_dashboard,,'time'=>$time
        ], compact('page_title', 'page_description','action'));
    }

    public function export_dashboard(){
        return Excel::download(new dashboard, 'dashboard.xlsx');
    }
	    
    public function obat()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Obat';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $kandungan_obat = DB::table('obat')->leftJoin('obat_kandungan_detail',
                                            'obat.id_obat',
                                            'obat_kandungan_detail.id_obat')
                                ->leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                                ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                                ->get();
        $table_obat = DB::table('obat')->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                                        ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                                        ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                                        ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                                        ->GroupBy('obat.id_obat')->where('obat.status','Continue')->get();
        
        $table_detail_obat = gudang_obat::rightJoin('detail_penerimaan_obat','stok_gudang.id_detail_penerimaan','detail_penerimaan_obat.id_detail_penerimaan_obat')
                                ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                                ->leftJoin('penerimaan','detail_penerimaan_obat.id_penerimaan','penerimaan.id_penerimaan')
                                ->get();    
        foreach($table_detail_obat as $data){
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }

        $dosis_obat = dosis_obat_detail::leftJoin('obat','dosis_obat_detail.id_obat','obat.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')->get();
        $satuan_dosis = dosis::all();
        $kandungan = kandungan::all();
        $golongan_obat = golongan_obat::all();
        $kategori_obat = kategori_obat::all();
        $pabrik_obat = pabrik_obat::all();
        $satuan_obat = satuan_obat::all();
        $satuan_obat_detail = satuan_obat_detail::all();
        $status = "Continue";
        // dd($table_detail_obat);
        return view('datamaster.obat',[
            'table_detail_obat'=>$table_detail_obat,
            'kategori_obat'=> $kategori_obat,
            'golongan_obat'=> $golongan_obat,
            'pabrik_obat'=> $pabrik_obat,
            'satuan_obat'=> $satuan_obat,
            'obat' => $kandungan_obat,
            'table_obat' => $table_obat,
            'dosis_obat' => $dosis_obat,
            'satuan_dosis' => $satuan_dosis,
            'satuan_obat_detail'=> $satuan_obat_detail,
            'kandungan'=>$kandungan_obat,
            'data_kandungan'=>$kandungan,
            'status'=>$status,
        ], 
            compact('page_title', 'page_description','action'));
    }
    public function obatDiscontinue()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Obat';
        $page_description = 'Some description for the page';
		
		$action = 'obat';

        $kandungan_obat = DB::table('obat')->leftJoin('obat_kandungan_detail',
                                            'obat.id_obat',
                                            'obat_kandungan_detail.id_obat')
                                ->leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                                ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                                ->get();
        $table_obat = DB::table('obat')->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                                        ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                                        ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                                        ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                                        ->GroupBy('obat.id_obat')->where('obat.status','Discontinue')->get();
        
        $table_detail_obat = gudang_obat::rightJoin('detail_penerimaan_obat','stok_gudang.id_detail_penerimaan','detail_penerimaan_obat.id_detail_penerimaan_obat')
                                ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                                ->leftJoin('penerimaan','detail_penerimaan_obat.id_penerimaan','penerimaan.id_penerimaan')
                                ->get();
        foreach($table_detail_obat as $data){
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }

        $dosis_obat = dosis_obat_detail::leftJoin('obat','dosis_obat_detail.id_obat','obat.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')->get();
        $satuan_dosis = dosis::all();
        $kandungan = kandungan::all();
        $golongan_obat = golongan_obat::all();
        $kategori_obat = kategori_obat::all();
        $pabrik_obat = pabrik_obat::all();
        $satuan_obat = satuan_obat::all();
        $satuan_obat_detail = satuan_obat_detail::all();
        $status = "Discontinue";
        return view('datamaster.obat',[
            'table_detail_obat'=>$table_detail_obat,
            'kategori_obat'=> $kategori_obat,
            'golongan_obat'=> $golongan_obat,
            'pabrik_obat'=> $pabrik_obat,
            'satuan_obat'=> $satuan_obat,
            'obat' => $kandungan_obat,
            'table_obat' => $table_obat,
            'dosis_obat' => $dosis_obat,
            'satuan_dosis' => $satuan_dosis,
            'satuan_obat_detail'=> $satuan_obat_detail,
            'kandungan'=>$kandungan_obat,
            'data_kandungan'=>$kandungan,
            'status'=>$status,
        ], 
            compact('page_title', 'page_description','action'));
    }

    public function stok()
    {
        $page_title = 'Stok';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_obat = obat::all();
        $data_stok = db::table('stok_obat')->leftJoin('obat','stok_obat.id_obat','obat.ID')->get();

        return view('datamaster.stok',['data_stok' => $data_stok, 'data_obat' => $data_obat], compact('page_title', 'page_description','action'));
    }

    public function golongan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Golongan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_golongan_obat = DB::table('obat')->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')->get();
        
        return view('datamaster.golongan',['data_golongan_obat'=>$data_golongan_obat], compact('page_title', 'page_description','action'));
    }
    public function kelolagolongan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $data_id = golongan_obat::max('id_golongan');
        $id = (int) substr($data_id, 4, 3);
        $id++;
        $id_golongan = 'gol-' . sprintf("%03s", $id);
        
        $page_title = 'Kelola Golongan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_golongan = golongan_obat::all();
        return view('datamaster.kelolagolongan',['data_golongan'=>$data_golongan,'id_golongan'=>$id_golongan], compact('page_title', 'page_description','action'));
    }

    public function katagori()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Katagori';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $kategori_obat = obat::leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                            ->get();
                            

        return view('datamaster.katagori',['kategori_obat'=>$kategori_obat], compact('page_title', 'page_description','action'));
    }
    public function kelolakatagori()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $data_id = kategori_obat::max('id_kategori');
        
        $id = (int) substr($data_id, 0, 3);
        $id++;
        $id_kategori = sprintf("%03s", $id);
        $page_title = 'Kelola Katagori';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_kategori = kategori_obat::all();

        return view('datamaster.kelolakatagori',['data_kategori'=>$data_kategori,'id_kategori'=>$id_kategori], compact('page_title', 'page_description','action'));
    }

    public function laporankatagori()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Laporan Katagori';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_kategori = kategori_obat::all();
        $data_golongan  = golongan_obat::all();
        
        $kandungan_obat = DB::table('obat')->leftJoin('obat_kandungan_detail','obat.id_obat','obat_kandungan_detail.id_obat')
                                ->leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                                ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                                ->get();
        $data_laporan_kategori = DB::table('obat')->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                                        ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                                        ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                                        ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                                        ->get();
        return view('datamaster.laporankatagori',['golongan'=>$data_golongan,'data_laporan'=>$data_laporan_kategori, 'kategori'=>$data_kategori], compact('page_title', 'page_description','action'));
    }

    public function stokOpnam()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $time = Carbon::now()             
                ->format('mY');
        $data_id = stok_opnam::where('id_stok_opnam','like','%'.$time.'%')->max('id_stok_opnam');
        $id = (int) substr($data_id, 13, 3);
        $id++;
        $id_stok_opnam = 'opm_'.$time .'_'. sprintf("%03s", $id);
        // $convert = \DateTime::createFromFormat('Y-m-d', $date_expired)->format('d F, Y');
        
        $data_gudang = gudang_obat::leftJoin('detail_penerimaan_obat','stok_gudang.id_detail_penerimaan','detail_penerimaan_obat.id_detail_penerimaan_obat')
                            ->get();
        
        $data_obat_tambah = detail_penerimaan_obat::leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                        ->leftJoin('penerimaan','detail_penerimaan_obat.id_penerimaan','penerimaan.id_penerimaan')
                        ->leftJoin('stok_gudang','detail_penerimaan_obat.id_detail_penerimaan_obat','stok_gudang.id_detail_penerimaan')
                        ->where('status','Continue')->where('stok_gudang.id_stok_opname',null)->get();
        $data_obat_edit = detail_penerimaan_obat::leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                        ->leftJoin('penerimaan','detail_penerimaan_obat.id_penerimaan','penerimaan.id_penerimaan')
                        ->leftJoin('stok_gudang','detail_penerimaan_obat.id_detail_penerimaan_obat','stok_gudang.id_detail_penerimaan')
                        ->where('status','Continue')->get();
        
        $data_stok_opnam = stok_opnam::leftJoin('detail_penerimaan_obat','stok_opnam.batch_number','detail_penerimaan_obat.batch_number')
                                ->leftJoin('stok_gudang','stok_opnam.id_stok_opnam','stok_gudang.id_stok_opname')
                                ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                                ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                                ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                                ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                                ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                                ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                                ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                                ->get();
        $kandungan_obat = DB::table('obat')->leftJoin('obat_kandungan_detail',
                                        'obat.id_obat',
                                        'obat_kandungan_detail.id_obat')
                            ->leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                            ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                            ->get();
        foreach($data_stok_opnam as $data){
            $data->expired_date = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }
        // dd($data_stok_opnam);
        $page_title = 'Penerimaan Opnam';
        $page_description = 'Some description for the page';
		
		$action = 'penerimaanOpnam';

        return view('datamaster.stokOpnam',[
        'id_stok_opnam'=>$id_stok_opnam,
        'tambah_obat'=>$data_obat_tambah,
        'edit_obat'=>$data_obat_edit,
        'data_stok_opnam'=>$data_stok_opnam,
        'data_gudang'=>$data_gudang
        ], compact('page_title', 'page_description','action'));
    }

    public function kandungan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Kandungan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_kandungan = kandungan::all();

        return view('datamaster.kandungan',['data_kandungan'=>$data_kandungan], compact('page_title', 'page_description','action'));
    }
    public function laporankandungan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };
        

        $page_title = 'Laporan Kandungan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_kategori = kategori_obat::all();
        $data_golongan  = golongan_obat::all();
        $obat = obat::leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                        ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->GroupBy('obat.id_obat')->get();
        $kandungan_obat = obat_kandungan_detail::leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                                        ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                                        ->get();
        $search_golongan='';
        $search_kategori='';
        $search_status='';
        // [
        //     'kategori',
        //     'status',
        //     'golongan',
        // ];
        // dd($search);
        return view('datamaster.laporankandungan',['kategori'=>$data_kategori,
            'search_kategori'=>$search_kategori,'search_golongan'=>$search_golongan,'search_status'=>$search_status,
            'golongan'=>$data_golongan,
            'obat'=>$obat,
            'kandungan_obat'=>$kandungan_obat],
            compact('page_title', 'page_description','action'));
    }
    public function export()
    {
        $page_title = 'Export Excel';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('datamaster.export', compact('page_title', 'page_description','action'));
    }

    public function dosis()
    {
        $data_id = dosis::max('id_dosis');
        $id = (int) substr($data_id, 4, 3);
        $id++;
        $id_dosis = 'dos_' . sprintf("%03s", $id);

        $page_title = 'Dosis';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_dosis = dosis::all();

        return view('datamaster.dosis',['data_dosis'=>$data_dosis,'id_dosis'=>$id_dosis], compact('page_title', 'page_description','action'));
    }

    public function satuan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $data_id = satuan_obat::max('id_satuan_obat');
        $id = (int) substr($data_id, 4, 3);
        $id++;
        $id_satuan = 'stn_' . sprintf("%03s", $id);

        $page_title = 'Satuan';
        $page_description = 'Some description for the page';

        $data_satuan_obat = satuan_obat::all();
		
		$action = __FUNCTION__;

        return view('datamaster.satuan',['data_satuan'=>$data_satuan_obat, 'id_satuan'=>$id_satuan], compact('page_title', 'page_description','action'));
    }

    public function pabrik()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Pabrik';
        $page_description = 'Some description for the page';
		
        $data_pabrik_obat = DB::table('pabrik')->select('pabrik.*')
                            ->leftJoin('obat','pabrik.id_pabrik','obat.id_pabrik')
                            ->groupBy('pabrik.id_pabrik')->get();
        $data_obat = DB::table('pabrik')
                    ->leftJoin('obat','pabrik.id_pabrik','obat.id_pabrik')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->get();
                    // dd($data_pabrik_obat);
        $kandungan_obat = DB::table('obat')->leftJoin('obat_kandungan_detail',
                    'obat.id_obat',
                    'obat_kandungan_detail.id_obat')
                    ->leftJoin('kandungan','obat_kandungan_detail.id_kandungan','kandungan.id_kandungan')
                    ->leftJoin('dosis','obat_kandungan_detail.id_dosis','dosis.id_dosis')
                    ->get();
        
		$action = __FUNCTION__;

        return view('datamaster.pabrik',['data_pabrik'=>$data_pabrik_obat,
                                        'data_obat'=>$data_obat,
                                        'kandungan_obat'=>$kandungan_obat], 
                                        compact('page_title', 'page_description','action'));
    }

    public function pbf()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };


        $page_title = 'PBF';
        $page_description = 'Some description for the page';

		
		$action = __FUNCTION__;
        $data_pbf = pbf::all();
        $penerimaan_pbf = penerimaan::leftJoin('detail_penerimaan_obat','penerimaan.id_penerimaan','detail_penerimaan_obat.id_penerimaan')
                    ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->get();
        // dd($penerimaan_pbf);
        foreach($penerimaan_pbf as $pbf ){
            $pbf->tanggal_expired_obat = \DateTime::createFromFormat('Y-m-d',$pbf->tanggal_expired_obat)->format('j F, Y');
        }
        // dd($data_pbf);
        return view('datamaster.pbf', [
            'data_pbf'=>$data_pbf,
            'penerimaan'=>$penerimaan_pbf
        ], compact('page_title', 'page_description','action'));
    }

    public function dokter()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Dokter';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_dokter = dokter::all();

        return view('datamaster.dokter',['data_dokter'=>$data_dokter], compact('page_title', 'page_description','action'));
    }

    public function halpengadaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Halaman Pengadaan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        
        $data_pengadaan = pengadaan::all();
        
        $pbf = pbf::all();
        return view('halaman.halpengadaan',['data_pengadaan'=>$data_pengadaan,'data_pbf'=>$pbf], compact('page_title', 'page_description','action'));
    }
    public function tambahpengadaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Penambahan Pengadaan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $data_pbf = pbf::all();
        $data_pabrik = pabrik_obat::all();
        $data_obat = obat::leftJoin('gudang_obat' ,'obat.ID' ,'gudang_obat.id_obat')->where('gudang_obat.id','!=','null')->get() ;
        
        return view('halaman.tambahpengadaan',['data_pbf'=>$data_pbf,'data_pabrik'=>$data_pabrik,'data_obat'=>$data_obat], compact('page_title', 'page_description','action'));
    }
    public function editpengadaan($id)
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Perubahan Pengadaan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_pabrik = pabrik_obat::all();
        $data_obat = obat::LeftJoin('gudang_obat','obat.ID' ,'gudang_obat.id_obat')->get();
        $data_pengadaan = pengadaan::where('id_pengadaan',$id)->get();
        $data_pengadaan_detail = DB::table('detail_pengadaan')->where('id_pengadaan',$id)->get();
        $data_pbf = pbf::all();

        return view('halaman.editpengadaan',['data_pabrik'=>$data_pabrik,'data_obat'=>$data_obat,'data_pbf'=>$data_pbf,'data_pengadaan'=>$data_pengadaan,'data_pengadaan_detail'=>$data_pengadaan_detail], compact('page_title', 'page_description','action'));
    }
    public function detailpengadaan($id)
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $data_pengadaan = DB::table('pengadaan')
                        ->leftJoin('pbf','pengadaan.pbf','pbf.id_pbf')
                        ->where('id_pengadaan',$id)
                        ->get();
        $data_pengadaan_detail = DB::table('detail_pengadaan')
                                ->where('id_pengadaan',$id)
                                ->leftJoin('pabrik','detail_pengadaan.pabrik','pabrik.id_pabrik')
                                ->LeftJoin('gudang_obat','detail_pengadaan.id_obat' ,'gudang_obat.id')
                                ->leftJoin('obat','gudang_obat.id_obat','obat.ID')
                                ->get();
        
        $page_title = 'Detail Pengadaan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('halaman.detailpengadaan',['data_pengadaan'=>$data_pengadaan,'data_pengadaan_detail'=>$data_pengadaan_detail], compact('page_title', 'page_description','action'));
    }

    public function halpengeluaran()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Halaman Pengeluaran';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_obat = obat::rightJoin('detail_penerimaan_obat','obat.id_obat','detail_penerimaan_obat.id_obat')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->get();
        
        $data_dokter = dokter::all();
        
        $data_pengeluaran = pengeluaran::groupBy('pengeluaran.id_pengeluaran')->get();
        $detail_pengeluaran = detail_pengeluaran_obat::leftJoin('pengeluaran','detail_pengeluaran_obat.id_pengeluaran','pengeluaran.id_pengeluaran')
                            ->leftJoin('dokter','pengeluaran.id_dokter','dokter.id_dokter')
                            ->leftJoin('detail_penerimaan_obat','detail_pengeluaran_obat.batch_number','detail_penerimaan_obat.batch_number')
                            ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                            ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                            ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                            ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                            ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                            ->get();
        // dd($data_pengeluaran);
        return view('halaman.halpengeluaran',
        ['data_dokter'=>$data_dokter,
        'data_obat'=>$data_obat,
        'data_pengeluaran'=>$data_pengeluaran, 
        'detail_pengeluaran'=>$detail_pengeluaran
        ],
        compact('page_title', 'page_description','action'));
    }

    public function tambah_pengeluaran()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Penambahan Pengeluaran';
        $page_description = 'Some description for the page';
		$action = __FUNCTION__;
        
        $data_dokter = dokter::all();
        $data_obat = obat::rightJoin('detail_penerimaan_obat','obat.id_obat','detail_penerimaan_obat.id_obat')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->where('expired_status','false')
                        ->where('obat.status','Continue')
                        ->get();
        $kategori_pengeluaran = kategori_pengeluaran::all();
        return view('halaman.tambahpengeluaran',[
            'dokter'=>$data_dokter,'obat'=>$data_obat,'kategori_pengeluaran'=>$kategori_pengeluaran
        ],compact('page_title', 'page_description','action'));
    }

    public function edit_pengeluaran($id)
    {
        if(!isset(auth::user()->role)){
            return redirect('/');}
        $page_title = 'Penambahan Pengeluaran';
        $page_description = 'Some description for the page';
		$action = __FUNCTION__;
        
        $pengeluaran = pengeluaran::where('pengeluaran.id_pengeluaran',$id)
                                ->get();
        $obat_pengeluaran = detail_pengeluaran_obat::leftJoin('pengeluaran','detail_pengeluaran_obat.id_pengeluaran','pengeluaran.id_pengeluaran')
                            ->leftJoin('detail_penerimaan_obat','detail_pengeluaran_obat.batch_number','detail_penerimaan_obat.batch_number')
                            ->where('pengeluaran.id_pengeluaran',$id)
                            ->get();
        foreach($obat_pengeluaran as $Obat_pengeluaran){
            $tata_cara = explode("X", $Obat_pengeluaran->tata_cara_pemakaian);
            $Obat_pengeluaran->tata_cara1 = $tata_cara[0];
            $Obat_pengeluaran->tata_cara2 = $tata_cara[1];
        }
        // dd($pengeluaran);
        $data_dokter = dokter::all();
        $data_obat = obat::rightJoin('detail_penerimaan_obat','obat.id_obat','detail_penerimaan_obat.id_obat')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->where('expired_status','false')
                        ->where('obat.status','Continue')
                        ->get();
        $kategori_pengeluaran = kategori_pengeluaran::all();
        return view('halaman.editpengeluaran',
        ['dokter'=>$data_dokter,
        'kategori_pengeluaran'=>$kategori_pengeluaran,
        'obat'=>$data_obat,
        'data_pengeluaran'=>$pengeluaran,
        'obat_pengeluaran'=>$obat_pengeluaran,]
        ,compact('page_title', 'page_description','action'));
    }

    public function halpenerimaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };
        $page_title = 'Halaman Penerimaan';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        

        $data_obat = obat::where('status','Continue')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')->get();
        $data_pbf   = pbf::all();
        $data_table = penerimaan::leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')->get();
        $data_detail = penerimaan::leftJoin('detail_penerimaan_obat', 'penerimaan.id_penerimaan', 'detail_penerimaan_obat.id_penerimaan')
                    ->leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')
                    ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                    ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                    ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                    ->get();
        foreach($data_detail as $data){
            // dd(Carbon::parse($data->tanggal_expired_obat)->format('j F, Y'));
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }

        $data_kandungan = kandungan::leftJoin('obat_kandungan_detail','kandungan.id_kandungan','obat_kandungan_detail.id_kandungan')->get();

        return view('halaman.halpenerimaan',['data_obat'=>$data_obat, 'kandungan'=>$data_kandungan, 'data_pbf'=>$data_pbf, 'data_table'=>$data_table, 'data_detail'=>$data_detail], compact('page_title', 'page_description','action'));
    }

    public function tambah_penerimaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Penambahan Penerimaan';
        $page_description = 'Some description for the page';
		$action = 'tambah_penerimaan';

        $data_obat = obat::where('status','Continue')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->where('status','Continue')
                        ->get();
        $data_pbf   = pbf::all();

        return view('halaman.tambahpenerimaan',
        ['data_obat'=>$data_obat, 'data_pbf'=>$data_pbf], 
        compact('page_title', 'page_description','action'));

    }
    public function edit_penerimaan($id)
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Edit Penerimaan';
        $page_description = 'Some description for the page';
		$action = 'tambah_penerimaan';

        $data_penerimaan = penerimaan::leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')->where('penerimaan.id_penerimaan',$id)->get();
        $data_detail = penerimaan::leftJoin('detail_penerimaan_obat', 'penerimaan.id_penerimaan', 'detail_penerimaan_obat.id_penerimaan')
                    ->leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')
                    ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                    ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                    ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                    ->leftJoin('stok_gudang','detail_penerimaan_obat.id_detail_penerimaan_obat','stok_gudang.id_detail_penerimaan')
                    ->where('penerimaan.id_penerimaan',$id)
                    ->get();
        foreach($data_detail as $data){
            if(isset($data->id_history_gudang)){
                $data->status_gudang = 'uploaded';
            }else{
                $data->status_gudang = 'unuploaded';
            }
        }
        // dd($data_detail);
        $data_obat = obat::where('status','Continue')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                        ->where('status','Continue')
                        ->get();
        // $cek_status = gudang_obat::
        
        $data_pbf   = pbf::all();
        $count_data = $data_detail->count();
        
        foreach($data_detail as $data){
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }

        return view('halaman.editpenerimaan',
        ['data_obat'=>$data_obat, 
        'data_pbf'=>$data_pbf,
        'count_data'=>$count_data,
        'data_detail'=>$data_detail,
        'data_penerimaan'=>$data_penerimaan
        ], 
        compact('page_title', 'page_description','action'));

    }

    public function lappengadaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };
        
        $page_title = 'Laporan Pengadaan Obat';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;
        $pbf = pbf::all();
        $data_pengadaan =  DB::table('pengadaan')
                            ->Join('detail_pengadaan','pengadaan.id_pengadaan','detail_pengadaan.id_pengadaan')
                            ->leftJoin('pabrik','detail_pengadaan.pabrik','pabrik.id_pabrik')
                            ->leftJoin('gudang_obat','detail_pengadaan.id_obat','gudang_obat.id')
                            ->leftJoin('obat','gudang_obat.id_obat','obat.ID')
                            ->leftJoin('satuan','obat.satuan_obat','satuan.id_satuan_obat')
                            ->get();
        

        return view('laporan.lappengadaan',["data_pengadaan"=>$data_pengadaan,'pbf'=>$pbf], compact('page_title', 'page_description','action'));
    }

    public function lappengeluaran()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Laporan Pengeluaran Obat';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_pengeluaran = detail_pengeluaran_obat::leftJoin('pengeluaran','detail_pengeluaran_obat.id_pengeluaran','pengeluaran.id_pengeluaran')
                            ->leftJoin('dokter','pengeluaran.id_dokter','dokter.id_dokter')
                            ->leftJoin('detail_penerimaan_obat','detail_pengeluaran_obat.batch_number','detail_penerimaan_obat.batch_number')
                            ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                            ->leftJoin('stok_gudang','detail_pengeluaran_obat.id_obat_pengeluaran','stok_gudang.id_detail_pengeluaran')
                            ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                            ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                            ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                            ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                            ->get();
        // dd($data_pengeluaran);
        return view('laporan.lappengeluaran',['data_pengeluaran'=>$data_pengeluaran], compact('page_title', 'page_description','action'));
    }
    public function lappengeluaranresep()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Laporan Pengeluaran Resep';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('laporan.lappengeluaranresep', compact('page_title', 'page_description','action'));
    }

    public function lappenerimaan()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Laporan Penerimaan Obat';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_obat = obat::where('status','Continue')
                        ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                        ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                        ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                        ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')->get();
        $data_pbf   = pbf::all();
        $data_table = penerimaan::leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')->get();
        $data_detail = penerimaan::leftJoin('detail_penerimaan_obat', 'penerimaan.id_penerimaan', 'detail_penerimaan_obat.id_penerimaan')
                    ->leftJoin('pbf','penerimaan.id_pbf','pbf.id_pbf')
                    ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                    ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                    ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                    ->get();
        
        foreach($data_detail as $data){
            $data->tanggal_expired_obat = Carbon::parse($data->tanggal_expired_obat)->format('j F, Y');
        }
        // dd($data_detail);

        return view('laporan.lappenerimaan',['data_table'=>$data_table,'data_detail'=>$data_detail,'pbf'=>$data_pbf], compact('page_title', 'page_description','action'));
    }
    public function lapobated()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };

        $page_title = 'Laporan Obat ED';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $obat_ed = detail_penerimaan_obat::leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                    ->leftJoin('stok_gudang','detail_penerimaan_obat.id_detail_penerimaan_obat','stok_gudang.id_detail_penerimaan')
                    ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                    ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                    ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                    ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                    ->leftJoin('kategori','obat.id_kategori','kategori.id_kategori')
                    ->leftJoin('golongan','obat.id_golongan','golongan.id_golongan')
                    ->leftJoin('pabrik','obat.id_pabrik','pabrik.id_pabrik')
                    ->where('expired_status','true')->get();
        foreach($obat_ed as $ed){
            $ed->tanggal_expired_obat = Carbon::parse($ed->tanggal_expired_obat)->format('j F, Y');
        }

        return view('laporan.lapobated',['obat_ed'=>$obat_ed], compact('page_title', 'page_description','action'));
    }

    public function lapOpnam()
    {
        if(!isset(auth::user()->role)){
            return redirect('/');
        };
        
        $page_title = 'Laporan Opnam';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        $data_laporan_opnam = stok_opnam::leftJoin('stok_gudang','stok_opnam.id_stok_opnam','stok_gudang.id_stok_opname')
                            ->leftJoin('detail_penerimaan_obat','stok_gudang.id_detail_penerimaan','detail_penerimaan_obat.id_detail_penerimaan_obat')
                            ->leftJoin('obat','detail_penerimaan_obat.id_obat','obat.id_obat')
                            ->leftJoin('dosis_obat_detail','obat.id_obat','dosis_obat_detail.id_obat')
                            ->leftJoin('dosis','dosis_obat_detail.id_dosis','dosis.id_dosis')
                            ->leftJoin('satuan_obat_detail','obat.id_obat','satuan_obat_detail.id_obat')
                            ->leftJoin('satuan','satuan_obat_detail.id_satuan_obat','satuan.id_satuan_obat')
                            ->get();
        return view('laporan.laporanOpnam',['data_stok_opnam'=>$data_laporan_opnam], compact('page_title', 'page_description','action'));
    }
    public function export_laporan_obatED(){
        return Excel::download(new laporan_obatED, 'laporan_obat_ED.xlsx');
    }
    public function export_laporan_opnam(){
        return Excel::download(new laporan_stok_opnam, 'laporan_opnam.xlsx');
    }
    public function export_laporan_penerimaan(){
        return Excel::download(new laporan_penerimaan, 'laporan_penerimaan.xlsx');
    }
    public function export_laporan_pengadaan(){
        return Excel::download(new laporan_pengadaan, 'laporan_pengadaan.xlsx');
    }
    public function export_laporan_pengeluaran(){
        return Excel::download(new laporan_pengeluaran, 'laporan_pengeluaran.xlsx');
    }


	    // Calender
    public function app_calender()
    {
        $page_title = 'Calender';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('app.calender', compact('page_title', 'page_description','action'));
    }
    
	    // Profile
    public function app_profile()
    {
        $page_title = 'Profile';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('app.profile', compact('page_title', 'page_description','action'));
    }
	    // Post Details
    public function post_details()
    {
        $page_title = 'Post Details';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('app.post_details', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Chartist
    public function chart_chartist()
    {
        $page_title = 'Chart Chartist';
        $page_description = 'Some description for the page';
        $action = __FUNCTION__;

        return view('chart.chartist', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Chartjs
    public function chart_chartjs()
    {
        $page_title = 'Chart Chartjs';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('chart.chartjs', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Flot
    public function chart_flot()
    {
        $page_title = 'Chart Flot';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('chart.flot', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Morris
    public function chart_morris()
    {
        $page_title = 'Chart Morris';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('chart.morris', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Peity
    public function chart_peity()
    {
        $page_title = 'Chart Peity';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('chart.peity', compact('page_title', 'page_description','action'));
    }
	
	    // Chart Sparkline
    public function chart_sparkline()
    {
        $page_title = 'Chart Sparkline';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('chart.sparkline', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Checkout
    public function ecom_checkout()
    {
        $page_title = 'Checkout';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('ecom.checkout', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Customers
    public function ecom_customers()
    {
        $page_title = 'Ecom Customers';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.customers', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Invoice
    public function ecom_invoice()
    {
        $page_title = 'Invoice';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.invoice', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Product Detail
    public function ecom_product_detail()
    {
        $page_title = 'Product Detail';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.productdetail', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Product Grid
    public function ecom_product_grid()
    {
        $page_title = 'Product Grid';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.productgrid', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Product List
    public function ecom_product_list()
    {
        $page_title = 'Product List';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.productlist', compact('page_title', 'page_description','action'));
    }
	
	    // Ecommerce Product Order
    public function ecom_product_order()
    {
        $page_title = 'Product Order';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ecom.productorder', compact('page_title', 'page_description','action'));
    }
	
	    // Email Compose
    public function email_compose()
    {
        $page_title = 'Compose';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('message.compose', compact('page_title', 'page_description','action'));
    }
	
	    // Email Inbox
    public function email_inbox()
    {
        $page_title = 'Inbox';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('message.inbox', compact('page_title', 'page_description','action'));
    }
	
	    // Email Read
    public function email_read()
    {
        $page_title = 'Read';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('message.read', compact('page_title', 'page_description','action'));
    }
	
	    // Form Editor Summernote
    public function form_editor_summernote()
    {
        $page_title = 'Summernote Editor';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('form.editorsummernote', compact('page_title', 'page_description','action'));
	}
	
	    // Form Element
    public function form_element()
    {
        $page_title = 'Form Element';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('form.element', compact('page_title', 'page_description','action'));
    }
	
	    // Form Pickers
    public function form_pickers()
    {
        $page_title = 'Form Pickers';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('form.pickers', compact('page_title', 'page_description','action'));
    }
	
	    // Form Validation Jquery
    public function form_validation_jquery()
    {
        $page_title = 'Form Validation';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('form.validationjquery', compact('page_title', 'page_description','action'));
    }
	
	    // Form Wizard
    public function form_wizard()
    {
        $page_title = 'Form Wizard';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('form.wizard', compact('page_title', 'page_description','action'));
    }
    
    	
	    // Map Jqvmap
    public function map_jqvmap()
    {
        $page_title = 'Map Jqvmap';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('map.jqvmap', compact('page_title', 'page_description','action'));
    }
	
	    // Page Error 400
    public function page_error_400()
    {
        $page_title = 'Page Error 400';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('page.error400', compact('page_title', 'page_description','action'));
    }
	
	    // Page Error 403
    public function page_error_403()
    {
        $page_title = 'Page Error 403';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('page.error403', compact('page_title', 'page_description','action'));
    }
	
	    // Page Error 404
    public function page_error_404()
    {
        $page_title = 'Page Error 404';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('page.error404', compact('page_title', 'page_description','action'));
    }
	
	    // Page Error 500
    public function page_error_500()
    {
        $page_title = 'Page Error 500';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('page.error500', compact('page_title', 'page_description','action'));
    }
	
	    // Page Error 503
    public function page_error_503()
    {
        $page_title = 'Page Error 503';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('page.error503', compact('page_title', 'page_description','action'));
    }
	
	    // Page Forgot Password
    public function page_forgot_password()
    {
        $page_title = 'Page Forgot Password';
        $page_description = 'Some description for the page';
        
        $action = __FUNCTION__;

        return view('page.forgot_password', compact('page_title', 'page_description','action'));
    }
	
	    // Page Lock Screen
    public function page_lock_screen()
    {
        $page_title = 'Page Lock Screen';
        $page_description = 'Some description for the page';
		
        $action = __FUNCTION__;

        return view('page.lockscreen', compact('page_title', 'page_description','action'));
    }
	
	    // Page Login
    public function page_login()
    {
        $page_title = 'Page Login';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('page.login', compact('page_title', 'page_description','action'));
    }
	
	    // Page Register
    public function page_register()
    {
        $page_title = 'Page Register';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('page.register', compact('page_title', 'page_description','action'));
    }
	
	    // Table Bootstrap Basic
    public function table_bootstrap_basic()
    {
        $page_title = 'Table Basic';
        $page_description = 'Some description for the page';
        $action = __FUNCTION__;
        return view('table.bootstrapbasic', compact('page_title', 'page_description','action'));
    }
	
	    // Table Datatable Basic
    public function table_datatable_basic()
    {
        $page_title = 'Table Datatable';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('table.datatablebasic', compact('page_title', 'page_description','action'));
    }
	    // UC Nestable.
    public function uc_nestable()
    {
        $page_title = 'Nestable';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.nestable', compact('page_title', 'page_description','action'));
    }
	    // UC Lightgallery.
    public function uc_lightgallery()
    {
        $page_title = 'Lightgallery';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.lightgallery', compact('page_title', 'page_description','action'));
    }
	
	    // UC NoUi Slider
    public function uc_noui_slider()
    {
        $page_title = 'Noui Slider';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.nouislider', compact('page_title', 'page_description','action'));
    }
	
	    // UC Select2
    public function uc_select2()
    {
        $page_title = 'Select2';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.select2', compact('page_title', 'page_description','action'));
    }
	
	    // UC Sweetalert
    public function uc_sweetalert()
    {
        $page_title = 'Sweetalert';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.sweetalert', compact('page_title', 'page_description','action'));
    }
	
	    // UC Toastr
    public function uc_toastr()
    {
        $page_title = 'Toastr';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('uc.toastr', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Accordion
    public function ui_accordion()
    {
        $page_title = 'Accordion';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.accordion', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Alert
    public function ui_alert()
    {
        $page_title = 'Alert';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.alert', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Badge
    public function ui_badge()
    {
        $page_title = 'Badge';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.badge', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Button
    public function ui_button()
    {
        $page_title = 'Button';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.button', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Button Group
    public function ui_button_group()
    {
        $page_title = 'Button Group';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.buttongroup', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Card
    public function ui_card()
    {
        $page_title = 'Card';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.card', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Carousel
    public function ui_carousel()
    {
        $page_title = 'Carousel';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.carousel', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Dropdown
    public function ui_dropdown()
    {
        $page_title = 'Dropdown';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.dropdown', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Grid
    public function ui_grid()
    {
        $page_title = 'Grid';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.grid', compact('page_title', 'page_description','action'));
    }
	
	    // Ui List Group
    public function ui_list_group()
    {
        $page_title = 'List Group';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.listgroup', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Media Object
    public function ui_media_object()
    {
        $page_title = 'Media Object';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.mediaobject', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Modal
    public function ui_modal()
    {
        $page_title = 'Modal';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.modal', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Pagination
    public function ui_pagination()
    {
        $page_title = 'Pagination';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.pagination', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Popover
    public function ui_popover()
    {
        $page_title = 'Popover';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.popover', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Progressbar
    public function ui_progressbar()
    {
        $page_title = 'Progressbar';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.progressbar', compact('page_title', 'page_description','action'));
    }
	
	    // Ui Tab
    public function ui_tab()
    {
        $page_title = 'Tab';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.tab', compact('page_title', 'page_description','action'));
    }
	

	    // Ui Typography
    public function ui_typography()
    {
        $page_title = 'Typography';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('ui.typography', compact('page_title', 'page_description','action'));
    }
	
	    // Widget Basic
    public function widget_basic()
    {
        $page_title = 'Widget';
        $page_description = 'Some description for the page';
		
		$action = __FUNCTION__;

        return view('widget.widget_basic', compact('page_title', 'page_description','action'));
    }
}
